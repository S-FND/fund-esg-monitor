...
export default function EditPortfolioCompany() {
  ...
  const getCompanyList = async (id) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}` + `/investor/companyInfo/${id}`, {
        method: "GET",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${localStorage.getItem("auth_token")}` },
      });
      if (!res.ok) {
  ...
  return (
    <>
      {editData ? (
        <EditCompanyForm company={editData} />
      ) : (
        <div className="p-4 text-center text-muted-foreground">Loading company data...</div>
      )}
    </>
