...
// ─────────────────────────────────────────────────────────────────────────────
// useComplianceScore.ts
// Usage: const result = calculateComplianceScore(items)
// ─────────────────────────────────────────────────────────────────────────────
...
export function calculateComplianceScore(
  ...
  const scored: ItemScore[] = csItems.map((item) => {
    ...
    if (isApplicable) {
      // §18: Closed = Fireside-verified complete. Score is 100 regardless of
      // when the item was completed — the investor accepted it, penalty does
      // not apply. This also prevents zero-timeline items from being penalised
      // purely because actualDate > targetDate (common for items closed post-deadline).
      if (csStatus === "Closed") {
        itemScore = 100;
      } else if (bufferEndDate) {
        if (comp.completionDate <= bufferEndDate) {
          itemScore = 100;
        } else {
          delayAfterBuffer = daysBetween(bufferEndDate, comp.completionDate);
...
// ─────────────────────────────────────────────────────────────────────────────
// Usage in a React component (no custom hook needed — just useMemo):
//
//   import { useMemo } from "react";
//   import { calculateComplianceScore } from "./useComplianceScore";
//
//   function Dashboard({ items }: { items: PlanItem[] }) {
//     const result = useMemo(() => calculateComplianceScore(items), [items]);
//     return <div>{result.overallScore}</div>;
//   }
//
// Calling calculateComplianceScore() directly inside useMemo avoids any
// possibility of "Invalid hook call" errors caused by duplicate React copies,
// calling the hook outside a component body, or build-tool/module duplication.
// ─────────────────────────────────────────────────────────────────────────────