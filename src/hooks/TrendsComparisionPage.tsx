...
import {  Company, Fund, Industry, QCategory, RevenueStage } from "@/types/esg";
import { usePortfolioRankingsFromData } from "./usePortfolioRankingsFromData";
import { KPIEntryInput } from "@/components/mis/cumulative-analytics/lib/portfolio-helpers/computePortfolioScores";
import { FeatureRowLite } from "@/components/mis/ComputeAnalyticsDashboardDataInput";
// import { Company } from "@/components/mis/cumulative-analytics/lib/portfolio-helpers";
>>>>>>> Stashed changes
...
=======
export const TrendsComparisonPage = ({ filters, newInsight = false,kpiEntries,companyFeatures,allCompanyProfiles }: TrendsComparisonPageProps) => {
...
=======
  ...
  let mainFilteredKpiEntries: {companyId: string; kpi_id: string; value: string | null; quarter: string; year: number; submitted_at: string}[] = [];
  // let filteredCompanyFeaturesA: FeatureRowLite[] = [];
  // let filteredCompanyProfilesA: Company[] = [];
  // let filteredCompanyProfilesB: Company[] = [];
  let companyProfiles: Company[] = [...allCompanyProfiles] ;

  if(filters && filters.fund) {
    companyProfiles= companyProfiles?.filter(c => c.fund === filters.fund) || [];
  }
  if(filters && filters.industry) {
    companyProfiles= companyProfiles?.filter(c => c.industry === filters.industry) || [];
  }
  if(filters && filters.revenueStage) {
    companyProfiles= companyProfiles?.filter(c => c.revenueStage === filters.revenueStage) || [];
  }
  if(filters && filters.firesidePOC) {
    companyProfiles= companyProfiles?.filter(c => c.fl === filters.firesidePOC) || [];
  }

  mainFilteredKpiEntries=kpiEntries?.filter(k => companyProfiles?.some(c => c.company_id === k.companyId)) || [];
  ...
  const { rankings: allRankingsA, isLoading: rankingsALoading } = usePortfolioRankingsFromData({
    year: periodAFilters.year,
    quarter: periodAFilters.quarter || "FY",
    period: periodAFilters.period,
    kpiEntries: mainFilteredKpiEntries,
  ...
  const { rankings: allRankingsB, isLoading: rankingsBLoading } = usePortfolioRankingsFromData({
    year: periodBFilters.year,
    quarter: periodBFilters.quarter || "FY",
    period: periodBFilters.period,
    kpiEntries: [],
    companyFeatureSettings: [],
    companyProfiles: [],
  });
