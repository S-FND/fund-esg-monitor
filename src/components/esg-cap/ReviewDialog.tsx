      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="space-y-4 py-4 overflow-y-auto flex-1">
          ...
          <div>
            ...
            {canEdit ? (
              <Input 
                value={editedItem.responsibility} 
                onChange={(e) => handleInputChange('responsibility', e.target.value)}
                className={isFieldChanged('responsibility') ? "border-orange-400" : ""}
              />
            ) : (
              <p>{editedItem.responsibility}</p>
            )}
            {isFieldChanged('responsibility') && (
              <p className="text-xs text-amber-600 mt-1">
                Previous: {originalItem?.responsibility}
              </p>
            )}
          </div>
