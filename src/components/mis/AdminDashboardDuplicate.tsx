...
import { AnalyticsFilters, AnalyticsDashboardData } from '@/hooks/useAnalyticsDashboardData';
// import { useAnalyticsDashboardDataFromData } from '@/hooks/useAnalyticsDashboardDataFromData';
...
// import { InsightTabDuplicate } from '@/components/analytics/InsightTabDuplicate';
// import { usePortfolioRankingsFromData } from '@/hooks/usePortfolioRankingsFromData';
...
import { useAnalyticsDashboardDataFromData } from '@/hooks/useAnalyticsDashboardDataFromData';
import { usePortfolioRankingsFromData } from '@/hooks/usePortfolioRankingsFromData';
import { InsightTabDuplicate } from '../analytics/InsightTabDuplicate';
...
const AdminDashboardDuplicate = () => {
    ...
    // ── Fetch raw universe at page level, then delegate to pure processor ──
    const { asOf } = useAsOf();
    const rawQuery = useQuery({
        queryKey: ['admin-dashboard-duplicate-raw', filters.year, filters.period, asOf?.month ?? 'live', asOf?.year ?? 'live'],
        queryFn: async () => {
            const START_YEAR = 2024;
            const years: number[] = [];
            for (let y = START_YEAR; y <= filters.year; y++) years.push(y);
            ...
            const featRows = featuresRes.data || [];

            // const { data: profileRows } = await supabase
            //     .from('company_profiles')
            //     .select('company_id, revenue_stage, industry');
            let profilesRes = await http.get<Company[]>('mis/company-profiles');
            if (profilesRes.error) throw new Error('Error fetching company profiles');
            const profileRows = profilesRes.data || [];

            const companyFeatureSettings = (featRows || []) as { companyId: string; feature_key: string; enabled: boolean }[];
            const companyProfiles = (profileRows || []) as Company[];
            return { kpiEntries: allEntries, companyFeatureSettings, companyProfiles };
        },
    });

    const isLoading = rawQuery.isLoading;
    const error = rawQuery.error as unknown;
    const data: AnalyticsDashboardData | undefined = useAnalyticsDashboardDataFromData({
        filters,
        kpiEntries: rawQuery.data?.kpiEntries ?? [],
        companyFeatureSettings: rawQuery.data?.companyFeatureSettings ?? [],
    });

    // ─── Period-aware rankings via the FromData hook (honours filters.period + filters.quarter) ──
    const { rankings: portfolioRankings, isLoading: rankingsLoading } = usePortfolioRankingsFromData({
        year: filters.year,
        period: filters.period,
        quarter: filters.quarter,
        kpiEntries: (rawQuery.data?.kpiEntries ?? []).filter(e => e.year === filters.year),
        companyFeatureSettings: rawQuery.data?.companyFeatureSettings ?? [],
        companyProfiles: rawQuery.data?.companyProfiles ?? [],
    });
    ...
    const { data: featureSettings } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (featuresRes.error) throw new Error('Error fetching feature settings');
    ...
    if (featureSettings) {
        for (const s of featureSettings) {
            ...
            featureEnabledMap.get(s.feature_key)!.add(s.companyId);
        }
    }
...
export default AdminDashboardDuplicate;
